<?php
	...
	class metaTags{
		...
		public function __construct($ver){	
			$this->Signature['pluginVersion'] = $ver;
			// echo '<pre>'; print_r($this->Signature);echo '<pre>';
			add_action(
				'admin_enqueue_scripts',
				array(&$this, 'admin_enqueue')
			);
			add_action(
				'admin_menu',
				array(&$this, 'menu')
		...
		public function admin_enqueue(){
			wp_enqueue_style(
				$this->Signature['pluginSlug'].'-admin-style',
				plugins_url('css/'.$this->Signature['pluginSlug'].'-admin.css',
				__FILE__)
			); 
			wp_enqueue_script(
				$this->Signature['pluginSlug'].'-admin-script',
				plugins_url('js/'.$this->Signature['pluginSlug'].'-admin.js',
				__FILE__)
			);
		}
		...
		public function enqueue(){
			wp_enqueue_style(
				$this->Signature['pluginSlug'].'-front-style',
				plugins_url('css/'.$this->Signature['pluginSlug'].'-front.css',
				__FILE__)
			);
			wp_enqueue_script(
				$this->Signature['pluginSlug'].'-front-js',
				plugins_url('js/'.$this->Signature['pluginSlug'].'-front.js',
				__FILE__)
			);
		}
